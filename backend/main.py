...
def run_with_fairlearn(df, protected_attr):
    ...
    if "response" not in df.columns:
        raise ValueError(
            "LLM output not found in 'response'. Cannot run Fairlearn."
        )

    df["binary"] = df["response"].apply(
    ...
    disparity = demographic_parity_difference(
        df["binary"], sensitive_features=group
    )
...
def run_with_deepchecks(df, protected_attr):
    ...
    if "response" not in df.columns:
        raise ValueError(
            "LLM output not found in 'response'. Cannot run Deepchecks."
        )
